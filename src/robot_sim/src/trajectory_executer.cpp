...
{
  max_velocity_ = 1.0;
  trajectory_sub_ = this->create_subscription<trajectory_msgs::msg::JointTrajectory>("/joint_trajectory", 10, (std::bind(&TrajectoryExecuter::callback, this, _1)));
  new_trajectory_ = false;
  executing_ = false;
  current_vels_.resize(robot_->getNumJoints(), 0.0);
  timer_ = this->create_wall_timer(std::chrono::milliseconds(10), std::bind(&TrajectoryExecuter::timerCallback, this));
  return true;
}

void TrajectoryExecuter::callback(const trajectory_msgs::msg::JointTrajectory::SharedPtr msg)
{
...
bool TrajectoryExecuter::targetReached()
{
  ...
  for (size_t i=0; i<robot_->getNumJoints(); i++) {
	...
	RCLCPP_ERROR(this->get_logger(),"Joint overshoot in trajectory executer");
	return true;
      }
    }
  }
...
void TrajectoryExecuter::timerCallback()
{
  boost::mutex::scoped_lock lock(mutex_);
