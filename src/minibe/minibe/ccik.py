...
class CCIK(Node):
    ...
    def get_cartesian_command(self, command: CartesianCommand):
        ...
        b_T_ee_des = self.transform_to_hom(command.x_target) #read desired transform from command
        q_cur = self.q_current
        joint_transforms, b_T_ee_cur = self.forward_kinematics(q_cur)
        q0_des = [] #secondary objective
        #assume all joints are revolute about a canonical axis

        #first, we compute delta_x. x_current is [0, 0, 0, 0, 0, 0] in its own coordinate frame
        #need to get transformation from current to desired position
        ...
        delta_x = np.concatenate((ee_cur_T_ee_des[:3, 3], rot_des))

        
        #Convert delta_x into v_ee by multiplying by gain and scaling appropriately
        GAIN = 2
        v_ee = delta_x * GAIN
        v_trans = np.linalg.norm(v_ee[:3])
        if(v_trans > 0.1):
            scale_factor = 0.1 / v_trans
            v_ee = v_ee * scale_factor
        v_rot = np.linalg.norm(v_ee[3:])
        if(v_rot > 1):
            scale_factor = 1 / v_rot
            v_ee = v_ee * scale_factor
        
        #get jacobian and pseudoinverse
        J = self.get_jacobian(b_T_ee_cur, joint_transforms)
        Jps = np.linalg.pinv(J,3.0e-2) #pseudoinverse while treating small singular values as 0
        q_dot_des = np.dot(Jps, v_ee)
        ...
        if(command.secondary_objective):
            ...
            q_dot_sec[0] = Q0_GAIN * (q0_target-q0_cur)
            Jp = np.linalg.pinv(J)
            Jp_J = np.dot(Jp, J)
            q_dot_null = np.dot((np.eye(Jp_J.shape[0]) - Jp_J), q_dot_sec)
            q_dot_des = q_dot_des + q_dot_null

        #scale to 1 rad/s max
        angular_vel = np.linalg.norm(q_dot_des)
        if(angular_vel > 1):
            q_dot_des = q_dot_des / angular_vel


        velocities = [float(i) for i in q_dot_des]
        self.joint_velocity_msg.name = self.joint_names
        self.joint_velocity_msg.velocity = velocities
